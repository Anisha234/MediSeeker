...
class TrfmSeq2seq(nn.Module):
    ...
    def _encode(self, src):
        ...
        # mean, max, first*2
        return output # np.hstack([np.mean(output, axis=0), np.max(output, axis=0), output[0,:,:], penul[0,:,:] ]) # (B,4H)
...
def parse_arguments():
    ...
    parser.add_argument('--n_worker', '-w', type=int, default=1, help='number of workers')
...
def main():
    ...
    dt = list(split2['train']['smiles'])
 #   dt = pd.read_csv(args.data)['canonical_smiles'].values
    print(vocab, dir(vocab))
    dataset = Seq2seqDataset(dt, vocab)
    test_size = 4
    ...
    ckpt_path = './.save/trfm_new_2_0.pkl'
    ...
    for b, sm in enumerate(test_loader):
        ...
        sm = torch.t(sm.cuda()) # (T,B)
        with torch.no_grad():
            print(vocab.from_seq(sm, join=True))
            output = model(sm) # (T,B,V)
            #v = model._encode(sm)
            #for i in range(220):
              #  print(v[i,0,0:16])

            z=(torch.argmax(output, dim=2)).squeeze()
        #
        # 
        # 
        # 
        ...
        # 
        #     print(z)
            print(vocab.from_seq(z, join=True))
