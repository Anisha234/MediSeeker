...
class TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size,dim_feedforward, nhead, out_size, n_layers, dropout=0.1):
        super(TrfmSeq2seq, self).__init__()
        self.in_size = in_size
        self.hidden_size = hidden_size
        self.embed = nn.Embedding(in_size, hidden_size)
        self.pe = PositionalEncoding(hidden_size, dropout)
        self.trfm = nn.Transformer(d_model=hidden_size, nhead=nhead, 
        num_encoder_layers=n_layers, num_decoder_layers=n_layers, dim_feedforward=dim_feedforward)
    ...
    def _encode(self, src):
        ...
        return output 
        ####
        m1 =torch.mean(output, axis=0)
        m2 = torch.max(output,axis=0).values
        #print(m1.size(), m2.size(), output[0,:,:].size(), penul[0,:,:].size())
        # mean, max, first*2
        return torch.hstack((m1,m2, output[0,:,:], penul[0,:,:] )) # (B,4H)
    ...
    def encode(self, src):
        ...
        if batch_size<=2048:
            ...
            while ed<batch_size:
                st += 100
                ed += 100
                out = torch.concat([out, self._encode(src[:,st:ed])], axis=0)
            return out
...
def parse_arguments():
    ...
    parser.add_argument('--data', '-d', type=str, default='data/tiny_corpus.txt', help='train corpus (.csv)')
...
# Drop rows where the length of the smiles string is longer than 120
 ...
 #   dt = pd.read_csv(args.data)['canonical_smiles'].values
    dataset = Seq2seqDataset(list(filtered_df['smiles']), vocab,seq_len=args.seq_len)
    test_size = 20000
    ...
    model = TrfmSeq2seq(len(vocab), args.hidden, dim_feedforward=args.hidden, nhead=4, out_size=len(vocab), n_layers=args.n_layer).cuda()
