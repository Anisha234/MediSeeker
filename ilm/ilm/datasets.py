...
def get_dataset(dataset, split, *args, data_dir=None, shuffle=False, limit=None, **kwargs):
  ...
  if dataset == Dataset.CUSTOM:
  ...
  elif dataset == Dataset.SMILES:
    d=smiles(split, data_dir)
    ...
    print(type(d), len(d), type(d[0]), len(d[0]))
    print(d[0])
...
def smiles(split, data_dir):
  fp = os.path.join(data_dir, '{}.txt'.format(split))
  print(fp)
  try:
    with open(fp, 'r') as f:
      entries = [split_method(e.strip()).split() for e in f.read().strip().split('\n\n\n')]
  except:
    raise ValueError('Could not load from {}'.format(fp))
  return entries
