...
def infill_with_ilm(
  ...
  num_predicted_spans = 0
  with torch.no_grad():
    device = next(model.parameters()).device
    context = torch.tensor(x + [_sep_id], dtype=torch.long, device=device).unsqueeze(0).repeat(num_infills, 1)
    #print(_end_span_id, type(_end_span_id))
    z = torch.Tensor(np.array([_end_span_id])).unsqueeze(0).repeat(num_infills,1)
    z=z.long().cuda()
   ...
   # print("Context", context)
    while context.shape[0] > 0:
     ...
     # print("Logits", logits.size())
      next_tokens = sample_from_logits(logits, nucleus=nucleus, temp=temperature, topk =topk)
     # print("Next tokens", next_tokens.size())
      context = torch.cat((context, next_tokens), dim=1)
     # print("Context", context.size())
     # print("Zsize",z.size())

      context = torch.cat((context, z), dim =1)
      #print("Context (after z)", context.size())
      # This is just a deterministic for loop now that we execute k times
     # num_predicted_spans = (context == _end_span_id).long().sum(dim=1)
      num_predicted_spans+=1
      ...
      # print("Terminate", terminate, terminate_expected, terminate_toolong, next_tokens)
