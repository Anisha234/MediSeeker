...
def update_tokenizer(additional_ids_to_tokens, tokenizer=DEFAULT_TOKENIZER):
  ...
  print("State", state, additional_ids_to_tokens)
  ...
  if tokenizer == Tokenizer.GPT2:
    ...
    vocab_size_before = len(state[0])
    for el in additional_ids_to_tokens:
      state[0].insert(el, additional_ids_to_tokens[el])
      state[1][additional_ids_to_tokens[el]] = el
    print(state[0])
    print(state[1])
    print(additional_ids_to_tokens.values())
    print(additional_ids_to_tokens.keys())
    vocab_size_after = len(state[0])
