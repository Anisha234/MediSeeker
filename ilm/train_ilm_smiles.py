...
#  print("Tok mask", tok_masks)
  ...
  for i, (mask, (context, answers)) in enumerate(contexts_and_answers):
    ...
    if len(example) > sequence_length:
      #print("Example greater than seq length", len(example))
  ...
  # print("Inputs and TTS", len(inputs), len(tts))
#  print("Truncated ", warning_to_count['Example longer than sequence length'], len(inputs))
  return inputs, tts
...
def masked_dataset_to_inputs_and_tts(
  ...
  print("Full dataset", num_docs)
  dataset = dataset
  num_docs = len(dataset)
  #print("Masked_dataset_to_inputs", split,type(dataset), dataset[0],type(dataset[0]), len(dataset[0]))
  ...
  #print(_GLOBAL_WORKER_TARGET)
  # Hack, we are hard coding number of examples to 16 here
  inputs=np.zeros((16*len(dataset),256), dtype=int)
  tts=np.zeros((16*len(dataset),256), dtype=int)
  #inputs=None
  #tts = None
  start_idx = 0
  for i,el in enumerate(dataset):
    docs_input_and_tts= _GLOBAL_WORKER_TARGET(el)
    #print(docs_input_and_tts[0].shape, docs_input_and_tts[1].shape)
    num_samp = docs_input_and_tts[0].shape[0]
    #print(start_idx)
    inputs[start_idx:start_idx+num_samp,:] = docs_input_and_tts[0]
    tts[start_idx:start_idx+num_samp,:] = docs_input_and_tts[1]

    start_idx +=num_samp
   # print(docs_input_and_tts[1].shape)
    #if tts is not None:
   #   inputs = np.concatenate([inputs, docs_input_and_tts[0]],axis=0)
    #  tts = np.concatenate([tts, docs_input_and_tts[1]],axis=0)
     # print(len(inputs))
    #else:
   #   inputs = docs_input_and_tts[0]
    #  tts = docs_input_and_tts[1]
    if i%1000==0:
      print("Data read", i, num_docs)
  #  print(i, docs_input_and_tts)
  
  # TODO: Don't bother doing all the work if we're not going to use it
  inputs = inputs[:start_idx,:]
  tts = tts[:start_idx,:]
  print("After", inputs.shape, tts.shape)
...
def train(args):
  ...
  print("Mask type to id", mask_type_to_id)
  ...
  if not args.eval_only:
    ...
    print("Train tt to count", train_tt_to_count)
  ...
  # Load model, now get model from nanoGPT
  print('Initializing model...')

  # model init
  meta_vocab_size=vocab_size
  block_size = args.train_sequence_length # context of up to 256 previous characters
...
# baby GPT model :)
  ...
  if args.train_from_scratch:
       ...
       # model_args['vocab_size'] = meta_vocab_size if meta_vocab_size is not None else 50304
        ...
        print(gptconf, type(gptconf))

        model = GPT(gptconf)
  else:
        print(f"Loading pretrained model from {args.pretrain_dir}")
        # resume training from a checkpoint.
        ckpt_path = os.path.join(args.pretrain_dir, 'ckpt.pt')
  ...
  if args.eval_only:
    ...
    for i, eval_batch in enumerate(eval_dataloader):
      with torch.no_grad():
        ...
        print('Eval only', eval_inputs[0])
        eval_logits, _ = model(eval_inputs,block_output=True)
    ...
    while True:
      ...
      for batch in train_dataloader:
        ...
        if int(elapsed / args.train_eval_secs) > num_save:
          ...
          eval_token_counts = defaultdict(int)
          eval_token_loss_sums = defaultdict(float)
          print("Eval data loader 601",eval_dataloader)
          for i, eval_batch in enumerate(eval_dataloader):
           # print("Eval", i, eval_batch)
            with torch.no_grad():
              eval_inputs, eval_tts = tuple(t.to(device) for t in eval_batch)
              
            #  outputs = model(eval_inputs)
            #  loss, eval_logits = outputs[:2] ##      
              outputs = model(eval_inputs, block_output=True)
             # print("Outputs",outputs.size())
              eval_logits = outputs
             # print(eval_inputs.size(), outputs.size() )
              eval_logits_relevant = eval_logits[:, :-1].contiguous().view(-1, eval_logits.shape[-1])
         ...
         #   model_to_save.config.to_json_file(out_fn_to_fp(CONFIG_NAME))
            torch.save(model_to_save.state_dict(), out_fn_to_fp('model.pt'))
        ...
        outputs = model(inputs, block_output=True)
        logits = outputs
#        logits, _ = model(inputs)
        ...
        if 0:
          ...
          print("Logits out",torch.argmax(logits[0].squeeze(), dim=1))
          print("Logits", logits.size(), logits_relevant.size())
          print('Labels Infill',labels_infill.size(), labels_infill[0])
          print('Labels Context',labels_context.size(), labels_context[0])
        ...
        if 0:
          print("Loss infill and context", loss_infill, loss_context)
        loss = loss_infill
        ...
        if int(elapsed / args.train_summary_secs) > num_summary:
          ...
          print("Loss context item", loss_context_item)
          print("Loss infill item", loss_infill_item)
          with torch.no_grad():
...
if __name__ == '__main__':
  ...
  model_args.add_argument('--model_name', type=str, choices=ilm.constants.ILM_MODEL_NAMES)
