...
class MaskSmilesType(Enum):
  ELEMENT = 1

class MaskSmiles(MaskFn):
  def __init__(self, p=0.5):
    self.p = p

  @classmethod
  def mask_types(cls):
    return list(MaskSmilesType)
  ...
  def mask(self, doc):
    ...
    for span_offset, char in enumerate(doc):
      #print(span_offset, char)
      if not char in ['<sos>'] and random.random() < self.p:
        span_type = MaskSmilesType.ELEMENT
        span_len = 1
